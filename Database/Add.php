<?php
    ...
    if(isset($_POST['submit-event'])){
        if(!empty($_POST['name']) && !empty($_POST['represent']) && !empty($_POST['date'])){
            
            $name = $_POST["name"];
            $describe = $_POST["represent "];
            $date = $_POST["date"];

            $query = "insert into event(id, name, represent , date) VALUES (0, '$name', '$represent', '$date')";
            $run = mysqli_query($conn, $query);
            if($run){
                echo "<script>
                            alert('Inserted data successfully'); 
